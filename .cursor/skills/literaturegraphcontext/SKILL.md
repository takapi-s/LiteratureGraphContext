...
description: >-
  ...
  review, related work, research gaps, paper comparison, or configuring the
  litgraph MCP server.
---
...
- Interpreting LGC MCP tools: `list_papers`, `find_research_gaps`, `build_literature_matrix`, etc.
- Drafting related work from graph context (outline + gaps + matrix; prose stays with the MCP client).
...
3. **Index pipeline** (run from project root):
   ...
   litgraph extract -y    # LLM structured extract (needs API key)
...
| `find_research_gaps` | Clustered gap candidates |
...
- **`extract` calls an external LLM**; confirm or use `litgraph extract -y` only when appropriate.
- When answering from graph data, **cite `paper_id`, title, page, section, and `evidence_text`** when available.
- Related work **draft prose** is out of scope for LGC itself; use `generate_related_work_outline`, `find_research_gaps`, and `build_literature_matrix` as context for the client model.
- `litgraph watch` defaults to scan → parse → build (no LLM); pass `--auto-extract` for full pipeline on file changes.
