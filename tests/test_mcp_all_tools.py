...
from litgraph.mcp.smoke_cases import deprecated_redirect_cases
...
@pytest.mark.parametrize("case", deprecated_redirect_cases(), ids=lambda c: c["tool"])
def test_mcp_deprecated_tool_redirect(project_tmp, monkeypatch, case):
    server = setup_mcp_project(project_tmp, monkeypatch)
    payload = mcp_call(server, case["tool"], case["args"])
    assert_tool_result(payload, case)
